...
class membership_index extends Controller {
	...
	private function bankList(){
		...
		$list = json_decode(file_get_contents($url,false, stream_context_create($GLOBALS['arrContextOptions'])), true);
		return $list[1];
	}
	...
	private function saveToDB($cmsArgs){
		...
		$result = json_decode(file_get_contents($url,false, stream_context_create($GLOBALS['arrContextOptions'])), true);
		return $result;
	}
