...
jQuery(document).ready(function() {
	...
	function layout_resize() {
		...
		if(Modernizr.canvas) {
			var _navi_svg = jQuery('svg#site-navigation-svg');
			sm.find('dl.navi-menu dd a').each(function(di) {
				var $this = jQuery(this);
				var a_width = $this.width();
				var a_height = $this.height();
				var clip_path_up_name = 'site-navi-cp-'+$this.attr('data-path-id')+'-up';
				var clip_path_down_name = 'site-navi-cp-'+$this.attr('data-path-id')+'-down';
				var clip_path_up = _navi_svg.find('clippath#'+clip_path_up_name);
				if(clip_path_up.length > 0) {
					var clip_poly_up = clip_path_up.find('polygon');
					clip_poly_up.attr('points','0,0 '+a_width+',0 '+a_width+','+a_height);
					var clip_poly_down = _navi_svg.find('clippath#'+clip_path_down_name+' polygon');
					clip_poly_down.attr('points','0,0 0,'+a_height+' '+a_width+','+a_height);
				}
			});
		}
