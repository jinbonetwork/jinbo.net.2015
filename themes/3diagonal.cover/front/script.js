			...
			if($this.find('.sub-title').length > 0) {
				...
				if(Modernizr.canvas) {
					...
					if(subtitle_svg_cnt <= ( section_subtitle_svg_animate.length - 1 ) ) {
						...
						var _pattern = _svg.image('themes/3diagonal.cover/images/halftone_background.png',0,0,700,300).pattern(0, 0, svg_width, svg_height);
						items.svg = _svg;
					...
					if( $this.find('img.subtitle-background').length > 0 ) {
						...
						var alternative_image = jQuery('<img src="./themes/3diagonal.cover/images/halftone_background-ie-'+alternative_image_idx+'.png" class="subtitle-background">');
